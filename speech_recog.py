...
filename = 'resources/chapter1.wav'
with contextlib.closing(wave.open(filename, 'r')) as f:
    frames = f.getnframes()
    rate = f.getframerate()
    duration = int(frames / float(rate))
AUDIO_FILE = path.join(path.dirname(path.realpath(__file__)), "resources/chapter1.wav")
...
try:
    ...
    f = open('recognised_text.txt', 'a')  # writing recognised audio to .txt file
    f.write(text)
    f.write(' ')
